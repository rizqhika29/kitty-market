...
async function main() {
  ...
  console.log("Account1 (owner/bettor):", account.address);
  ...
  fs.writeFileSync(path.join(__dirname, "contract-address.json"),
    JSON.stringify({ address: CONTRACT, privateKey: pk, privateKey2: account2PK }, null, 2));
  ...
  // ========== WRITE: TAKE SIDE (from owner, not creator) ==========
  console.log("\n========== WRITE: TAKE SIDE ==========");
  ...
  // ========== WRITE: CASH OUT ==========
  console.log("\n========== WRITE: CASH OUT ==========");
  await writeAndWait("cash_out(1 GEN)", () => client.writeContract({
    address: CONTRACT, functionName: "cash_out", args: [BigInt(1_000_000_000_000_000_000)]
  }));

  // ========== WRITE: COLLECT FEES (from owner) ==========
  console.log("\n========== WRITE: COLLECT FEES ==========");
  await writeAndWait("collect_fees (from owner, expect fail - zero balance)", () => client.writeContract({
